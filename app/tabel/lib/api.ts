...
export async function getAllSekolahData(options: FetchAllOptions = {}): Promise<any[]> {
  ...
  console.log('🚀 Memulai fetch SEMUA data sekolah Jawa Timur...')
    ...
    while (!isCompleted && currentPage <= maxPages) {
      try {
        ...
        const response = await fetchWithRetry(
          `${API_BASE_URL}/sekolah?${searchParams}`,
          {
            retries: 3,
            timeout: 10000
          }
        ...
        if (currentPageData.length === 0) {
          ...
          // Delay kecil antar halaman untuk tidak membebani API
...
async function fetchWithRetry(url: string, options: { retries: number; timeout: number }) {
  ...
  for (let i = 0; i <= options.retries; i++) {
    try {
      ...
      console.log(`🔄 Retry ${i + 1}/${options.retries + 1} untuk ${url}`);
