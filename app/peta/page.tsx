...
export default function PetaPage() {
      ...
      try {
        ...
        const data = await getAllSekolahData({
          ...
          onProgress: (progress) => {
            ...
            const estimatedTotal = Math.max(27614, progress.totalFetched + 1000) // Update estimasi
        ...
        setLoadingProgress(prev => ({
          ...prev,
          estimatedTotal,
          message: `Selesai! ${data.length.toLocaleString('id-ID')} sekolah loaded`
        }))
  ...
  const progressPercentage = useMemo(() => {
    ...
    return Math.min(100, (loadingProgress.fetchedCount / loadingProgress.estimatedTotal) * 100)
  }, [loadingProgress])
      ...
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-100 flex items-center justify-center p-4">
        <div className="text-center max-w-lg mx-auto p-8 bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-200">
          <div className="relative w-40 h-40 mx-auto mb-8">
            ...
            {/* Progress ring */}
            <div 
              className="absolute inset-0 border-8 border-blue-600 rounded-full border-t-transparent border-r-transparent"
              style={{ 
          ...
          <div className="space-y-4 mb-6">
            <div>
              <div className="flex justify-between text-sm text-gray-700 mb-1">
                <span>Halaman {loadingProgress.currentPage} dari {loadingProgress.totalPages}</span>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-blue-50 p-3 rounded-lg">
                <div className="font-medium text-blue-700">Diambil</div>
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center p-8 max-w-md">
          ...
          <h3 className="text-xl font-bold text-gray-800 mb-2">Terjadi Error</h3>
          <p className="text-gray-600 mb-4">{error}</p>
          <p className="text-sm text-gray-500 mb-6">
            Data yang berhasil diambil: {allSchools.length.toLocaleString('id-ID')} sekolah
          </p>
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between">
            <div>
              ...
              <p className="text-blue-200 text-sm mt-1">
                {filteredSchools.length.toLocaleString('id-ID')} sekolah ditampilkan
                {' '}({allSchools.length.toLocaleString('id-ID')} total)
              </p>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-2">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-600">
              <span className="font-medium">{allSchools.length.toLocaleString('id-ID')}</span> total •{' '}
              <span className="font-medium text-blue-600">{filteredSchools.length.toLocaleString('id-ID')}</span> ditampilkan
              {filters.kabupaten.length > 0 && (
                <span className="ml-2">• {filters.kabupaten.length} kabupaten</span>
              )}
            </div>
          </div>
        </div>
